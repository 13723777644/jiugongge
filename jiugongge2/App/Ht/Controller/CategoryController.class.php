...
class CategoryController extends PublicController{
    ...
    */
    public function _initialize(){
        ...
        foreach ($list as $k1 => $v1) {
            $list[$k1]['list2'] = $this->category->where('tid='.intval($v1['id']))->field('id,tid,name,bz_2,bz_4')->select();
            foreach ($list[$k1]['list2'] as $k2 => $v2) {
                $list[$k1]['list2'][$k2]['list3'] = $this->category->where('tid='.intval($v2['id']))->field('id,tid,name,bz_2,bz_4')->select();
            }
        }
    ...
    */
    public function set_jy(){
        ...
        $data=array();
        $data['bz_4'] = $cate_info['bz_4'] == 1 ?  0 : 1;
        $up = $this->category->where('id='.intval($id))->save($data);
        if ($up) {
            $this->success('操作成功.');
        }else{
            $this->error('操作失败.');
