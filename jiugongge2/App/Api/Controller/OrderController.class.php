...
class OrderController extends PublicController {
    ...
    public function myorder(){
        $uid=I("post.uid");
        $ptype=I('post.ptype');
        $where['outtime']=array('elt',time());
        $where['uid']=array('eq',$uid);
        $kan=M('kanjia')->where($where)->select();
        $kanjialist=M('kanjialist');
        foreach ($kan as $k=>$v){
           $kanjialist->where(['kid'=>$v['id'],'status'=>0])->delete();

        }


        if($ptype==1){//代销费
            $order=   M('order')->where(['uid'=>$uid,'del'=>0,'status'=>10])->order('addtime desc')->select();
        }elseif($ptype==2){
            $order=   M('order')->where(['uid'=>$uid,'del'=>0,'status'=>20])->order('addtime desc')->select();
        }
        foreach ($order as $k=>$v){
            $order[$k]['goods']=M('product')->where(['id'=>$v['pid'],'del'=>0])->find();
            $order[$k]['goods']['photo']= __DATAURL__.$order[$k]['goods']['photo'];
            $order[$k]['addtime']=date('Y-m-d H:i:s',$v['addtime']);
        }
    $tel=M('program')->where(['id'=>1])->getField('phone');

        echo json_encode(array('status' => 1,'order'=>$order,'tel'=>$tel));
        exit();
    }
    ...
    public function delorder(){
        ...
        M('order')->where(['id'=>$oid])->save(['del'=>1]);
    ...
    public function chongzhi(){
        ...
        $data['xunibisn']=$this->build_order_no();//生成唯一订单号
        $data['uid']=$uid;
        $data['addtime']=time();
        $data['amount']=I('post.zhi');
        $data['price']=I('post.zhi');
       $res= M('chongzhi')->add($data);
       if($res){
           $arr = array();
           $arr['order_id'] = $res ;
           $arr['order_sn'] = $data['xunibisn'];
           $arr['pay_type'] = "weixin";
           //  $arr['type']= $data['type'];
    ...
    public function index(){
        ...
        $order = $orders->where($condition)->order('id desc')->field('id,order_sn,status,price,amount,type,product_num')->limit($pages.',7')->select();
    ...
    public function order_details(){
        ...
        $order_info = $orders->where('id='.intval($order_id.' AND del=0'))->field('id,addid,order_sn,status,addtime,amount,type,tel,receiver,kuaidi_name,addid,kuaidi_num,liuyan')->find();
...
//        }
       ...
       $adds= M('address')->where(['id'=>$order_info['addid']])->find();

        echo json_encode(array('status'=>1,'pro'=>$pro,'ord'=>$order_info,'adds'=>$adds));
        exit();
    }
    ...
    public function change_order(){
        ...
        if(!$shopid){
            echo json_encode(array("status"=>0,"err"=>"您尚未开通店铺!"));exit;
        }
        $orderid=I("post.orderid");
        $status=I("post.status");
        $data['kuaidi_name']=I("post.kuaidi_name");
        $data['kuaidi_num']=I("post.kuaidi_num");
        $statusarr=array(
            "0"=>20,
            "1"=>30,
            "2"=>40,
        ...
        $check=M("order")->where("id=$orderid")->getField("status");
        if($check>20){
            if($check>$statusarr[$status]){
                echo json_encode(array("status"=>0,"err"=>"状态无法倒退!"));exit;
            }
        }
        $re=M("order")->where("id=$orderid AND shop_id=$shopid")->save($data);
        if($re){
            echo json_encode(array("status"=>1,"err"=>"状态更改成功!"));
        }else{
            echo json_encode(array("status"=>0,"err"=>"状态更改失败!"));
        }
